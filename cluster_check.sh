#!/bin/bash
# One-shot diagnostic to run on the LOGIN node of a SLURM cluster (NOT a GPU node).
# Usage:
#   cd <project_root>
#   bash cluster_check.sh 2>&1 | tee cluster_check.log
# Reports Python / pip versions, HF cache presence, adapter location, and
# network reachability so we can decide between merged-model mode and adapter mode.
...
if [ -d "$ADAPTER_DIR" ]; then
    ...
    echo "$ADAPTER_DIR missing (did you pull the latest commit and place the adapter?)"
fi
echo ""
